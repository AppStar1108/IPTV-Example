...
import com.redorange.motutv1.db.OperationLive.AddToDeleteListener;
...
public class OperationLock {
	 SQLiteDatabase db;
	  ...
	  public OperationLock(Context paramContext)
	  {
	  ...
	  public void addToDeleteLive(Channel paramChannel, AddToDeleteListener paramAddToDeleteListener)
	  {
	    ...
	    long l = this.db.insert("lock", null, localContentValues);
	    if ((this.db != null) || (this.db.isOpen()))
	      this.db.close();
	  ...
	  public int deleteVod(int paramInt)
	  {
	    ...
	    int i = localSQLiteDatabase.delete("lock", "idTv = ?", arrayOfString);
	    if ((this.db != null) || (this.db.isOpen()))
	      this.db.close();
	    return i;
	  }
	  ...
	  public boolean isInsert(int paramInt)
	  {
	    ...
	    String[] arrayOfString = new String[1];
	    arrayOfString[0] = String.valueOf(paramInt);
	    Cursor localCursor = localSQLiteDatabase.rawQuery("select * from lock where idTv = ?", arrayOfString);
	  ...
	  public List<Channel> queryAllLive()
	  {
	    ...
	    Cursor localCursor = this.db.rawQuery("select * from lock", null);
	  ...
	  public boolean queryIsHaveData()
	  {
	    if ((this.db == null) || (!this.db.isOpen()))
	      this.db = this.dbHelper.getWritableDatabase();
	    return this.db.rawQuery("select * from lock", null).moveToNext();
	  }
